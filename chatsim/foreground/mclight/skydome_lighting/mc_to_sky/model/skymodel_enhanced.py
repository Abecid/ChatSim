"""
Peak Direction and Intensity enhanced sky model (with peak residual connection.)
"""
...
class SkyModelEnhanced(pl.LightningModule):
    ...
    def decode_forward(self, latent_vector, peak_vector, peak_vector_gt):
        ...
        # enhance with peak intensity
        sum_mask = sum_mask.permute(0,3,1,2) # N, 3, H, W
        sun_peak_map = peak_dir_encoding.permute(0,3,1,2) * peak_int_encoding.permute(0,3,1,2) # [N,3,H,W]
    ...
    def build_peak_map(self, peak_vector):
        ...
        peak_dir_cosine = torch.einsum('nhwc,nhwc->nhw', dir_vector_expand, self.pos_encoding.expand(dir_vector_expand.shape)).unsqueeze(-1) # B, H, W, 1
        peak_dir_encoding = torch.exp(100*(peak_dir_cosine - 1)) # B, H, W, 1

        sun_mask = torch.gt(peak_dir_encoding, self.sum_lobe_thres).expand(-1,-1,-1,3) # [B, H, W, 3]
